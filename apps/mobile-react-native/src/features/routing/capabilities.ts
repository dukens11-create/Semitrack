...
export const capabilitiesSchema = z.object({
  ...
  truckRouting:z.object({provider:z.literal('Trimble'),status:z.enum(['OPERATIONAL','DEGRADED','UNAVAILABLE','NOT_CONFIGURED']),state:z.enum(['PLANNING_AVAILABLE','PLANNING_UNVERIFIED','PROVIDER_UNAVAILABLE','PROVIDER_MISCONFIGURED']),requestAllowed:z.boolean(),verifiedProfileRequired:z.literal(true),validUntil:z.string().nullable()}),
  turnByTurn:z.object({available:z.literal(false),state:z.literal('UNAVAILABLE'),licenseStatus:z.literal('UNVERIFIED')}),
});
...
export function planningStatus(input:{verified:boolean;phase:string;errorCode?:string;capabilities?:Capabilities;now?:number}) {
 ...
 if(input.phase==='error')return 'Route calculation failed — review the error and retry';
 if(input.phase==='preview')return 'Truck route calculated · Navigation not started';
 const route=input.capabilities?.truckRouting;
 if(route?.state==='PROVIDER_MISCONFIGURED')return 'Trimble routing configuration needs attention';
 if(route?.state==='PROVIDER_UNAVAILABLE')return 'Trimble routing temporarily unavailable';
 const fresh=route?.validUntil && Date.parse(route.validUntil)>(input.now??Date.now());
 if(route?.status==='OPERATIONAL' && fresh)return 'Truck route planning available · Choose a destination';
